...
# Doro installer - Jesse's AI Coding Agent
# Usage: curl -fsSL https://raw.githubusercontent.com/ChiTing111/doro/main/install.sh | bash
...
REPO="https://github.com/ChiTing111/doro.git"
INSTALL_DIR="$HOME/doro"
...
ART
  printf "${RESET}"
  printf "${DIM}  Doro - Jesse's AI Coding Agent${RESET}\n"
  echo ""
}
...
build_binary() {
  info "Building doro (all experimental features enabled)..."
  cd "$INSTALL_DIR"
...
link_binary() {
  ...
  ln -sf "$INSTALL_DIR/cli-dev" "$link_dir/doro"
  ok "Symlinked: $link_dir/doro"
...
printf "    ${CYAN}doro${RESET}                          # interactive REPL\n"
printf "    ${CYAN}doro -p \"your prompt\"${RESET}          # one-shot mode\n"
echo ""
...
printf "  ${DIM}Link:   ~/.local/bin/doro${RESET}\n"
echo ""
